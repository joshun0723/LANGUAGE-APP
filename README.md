...
4. 目前有三個畫面：
   - **首頁**
   - **單字卡**：點卡片看中文意思／例句，可個別播放單字發音和例句發音，也可以標記 ⭐ 重要單字（右上角點一下），並切換「只看標記」模式複習
   - **全大題限時模擬測驗**（12題，涵蓋單字4題／文法3題／閱讀3題／聽力2題，每題依題型限時15-30秒，練習多益後段答題節奏；聽力題會自動朗讀短文，可重播）
...
## 已知限制（Phase 1）
- 沒有雲端帳號系統，學習紀錄不會跨裝置同步（測驗結果只在單次使用時顯示；標記的單字是存在手機本機，換手機或清除 App 資料會重置）
- repo 需要是 Public 才能讓語音功能運作（詳見上方第2節說明）
- 模擬測驗的聽力題是用手機內建語音現場朗讀（跟單字卡發音同一個技術），不是額外音檔
